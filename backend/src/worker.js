...
const TIMELINE_VALUES = new Set(['先沟通评估', '一周内', '两周内', '一个月内', '时间灵活']);
const BUDGET_VALUES = new Set(['先沟通评估', '¥300–800', '¥800–1500', '¥1500–3000', '¥3000 以上']);
const STATUS_VALUES = new Set(['new', 'quoted', 'awaiting_payment', 'paid', 'in_progress', 'completed', 'cancelled']);
...
async function createOrder(request, env) {
  ...
  if (need.length < 10) return json(request, env, { error: '请至少填写 10 个字的需求说明' }, 400);
...
async function login(request, env) {
  assertAllowedOrigin(request, env);
  assertJson(request);
  if (!env.ADMIN_PASSWORD || !env.SESSION_SECRET) return json(request, env, { error: 'Admin is not configured' }, 503);
  const fingerprint = await sha256(request.headers.get('CF-Connecting-IP') || 'unknown');
  const now = Date.now();
  const row = await env.DB.prepare('SELECT window_started_at, attempts FROM login_attempts WHERE fingerprint = ?').bind(fingerprint).first();
  const inWindow = row && now - row.window_started_at < 15 * 60 * 1000;
  if (inWindow && row.attempts >= 5) return json(request, env, { error: '登录尝试过多，请稍后再试' }, 429);
  ...
  if (!constantTimeEqual(String(body.password || ''), env.ADMIN_PASSWORD)) {
    ...
    await env.DB.prepare('INSERT INTO login_attempts (fingerprint, window_started_at, attempts) VALUES (?, ?, ?) ON CONFLICT(fingerprint) DO UPDATE SET window_started_at = excluded.window_started_at, attempts = excluded.attempts').bind(fingerprint, start, attempts).run();
  ...
  const token = await signSession(env.SESSION_SECRET, Math.floor(Date.now() / 1000) + 8 * 60 * 60);
  return json(request, env, { ok: true }, 200, { 'Set-Cookie': `zj_admin=${token}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=28800` });
}

async function logout(request, env) {
  assertAllowedOrigin(request, env);
  return json(request, env, { ok: true }, 200, { 'Set-Cookie': 'zj_admin=; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=0' });
}
...
async function listOrders(request, env, url) {
  ...
  const query = status && STATUS_VALUES.has(status)
...
async function updateOrder(request, env, id) {
  ...
  const result = await env.DB.prepare('UPDATE orders SET status = ?, admin_note = ?, updated_at = ? WHERE id = ?')
    .bind(status, note, new Date().toISOString(), id).run();
  return result.meta.changes ? json(request, env, { ok: true }) : json(request, env, { error: 'Order not found' }, 404);
}

function assertAllowedOrigin(request, env) {
  const origin = request.headers.get('Origin');
  const workerOrigin = new URL(request.url).origin;
  if (origin !== env.PUBLIC_ORIGIN && origin !== workerOrigin) throw new HttpError(403, 'Origin denied');
}
...
async function readJson(request, maxBytes) {
  ...
  if (new TextEncoder().encode(text).length > maxBytes) throw new HttpError(413, 'Payload too large');
...
function enumValue(value, allowed, fallback) { return allowed.has(String(value || '')) ? String(value) : fallback; }
async function requireAdmin(request, env) {
  const cookie = request.headers.get('Cookie') || '';
  const token = cookie.split(';').map(v => v.trim()).find(v => v.startsWith('zj_admin='))?.slice(9);
  if (!token || !(await verifySession(token, env.SESSION_SECRET))) throw new HttpError(401, 'Unauthorized');
}
...
async function verifySession(token, secret) {
  ...
  if (!payload || !signature || !constantTimeEqual(signature, await hmac(secret, payload))) return false;
  try { const data = JSON.parse(atob(payload)); return data.role === 'admin' && data.exp > Date.now() / 1000; } catch { return false; }
}
...
function corsResponse(request, env, body, status, extra = {}) {
  const origin = request.headers.get('Origin');
  const headers = new Headers({ 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer', 'Content-Security-Policy': "default-src 'none'; frame-ancestors 'none'", ...extra });
  if (origin === env.PUBLIC_ORIGIN) { headers.set('Access-Control-Allow-Origin', origin); headers.set('Vary', 'Origin'); headers.set('Access-Control-Allow-Credentials', 'true'); headers.set('Access-Control-Allow-Headers', 'Content-Type'); headers.set('Access-Control-Allow-Methods', 'GET, POST, PATCH, OPTIONS'); }
  return new Response(body, { status, headers });
}
function json(request, env, data, status = 200, extra = {}) { return corsResponse(request, env, JSON.stringify(data), status, extra); }
class HttpError extends Error { constructor(status, message) { super(message); this.status = status; } }

function htmlHeaders() { return { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer', 'X-Frame-Options': 'DENY', 'Content-Security-Policy': "default-src 'self'; script-src 'self'; style-src 'unsafe-inline'; img-src 'none'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'" }; }
